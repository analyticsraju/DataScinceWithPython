...
to_delete_train = total_missing_train[(total_missing_train/n_train) > 0 ]#cutoff10%missing data
...
filterFeatures(train_updtd, ['SalePrice'])
X_train=train_updtd
X_train.info()
...
nnr_estimator = neural_network.MLPRegressor(random_state=2017,solver='sgd',activation='relu',hidden_layer_sizes=(150, 50))
nnr_grid = { 'alpha':[0.0001,0.001,0.01,0.1]}
grid_nnr_estimator = model_selection.GridSearchCV(nnr_estimator,nnr_grid,scoring=metrics.make_scorer(rmse),cv=10,n_jobs=1)
grid_nnr_estimator.fit(X_train, y_train)
print(grid_nnr_estimator.grid_scores_)
print(grid_nnr_estimator.best_params_)
print(grid_nnr_estimator.best_score_)
print(grid_nnr_estimator.score(X_train, y_train))
estimator = grid_nnr_estimator.best_estimator_
estimator.coefs_
...
test['SalePrice'] = grid_nnr_estimator.predict(X_test)

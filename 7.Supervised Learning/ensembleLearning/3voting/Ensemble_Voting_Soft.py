...
from EnsembleStackingDef import StackEnsemble
...
#Here preparting estimator with thre Models
hard_voting_estimator = ensemble.VotingClassifier(estimators=[('rf', rf_estimator), ('gbm', gbm_estimator),('lr', lr_estimator)], voting='soft')

#Here preparing voting grid with 3 models parameters.
voting_grid={'rf__n_estimators':[100,200],'rf__max_features':[2,3],'gbm__n_estimators':[100,150],'gbm__max_depth':[3,5],'gbm__learning_rate':[0.05],'lr__C':[0.4,0.1]}
grid1 = model_selection.GridSearchCV(estimator=hard_voting_estimator,param_grid=voting_grid,n_jobs=1,cv=10)
#Fitting on complete data
grid1.fit(X_train,y_train)
grid1.grid_scores_

#Same above but for Soft Voting
soft_voting_estimator = ensemble.VotingClassifier(estimators=[('rf', rf_estimator), ('gbm', gbm_estimator),('lr', lr_estimator)], voting='soft')
grid2 = model_selection.GridSearchCV(estimator=soft_voting_estimator,param_grid=voting_grid,n_jobs=1,cv=10)
grid2.fit(X_train,y_train)
grid2.grid_scores_

X_test = titanic2[titanic_train.shape[0]:]
titanic_test['Survived'] = grid2.predict(titanic2[titanic_train.shape[0]:])

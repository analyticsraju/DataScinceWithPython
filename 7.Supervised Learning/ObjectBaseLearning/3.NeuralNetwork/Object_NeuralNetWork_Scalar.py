...
#Scalar: Scalar will give equal importance/weightage to all features
#transform all the data to common scale. sothat no single feature dominating learning
nn_scalar = preprocessing.StandardScaler()
nn_scalar.fit(X_train)
X_train = nn_scalar.transform(X_train)

#Model Building with NeuralNetwork Algorithm
nn_estimator = neural_network.MLPClassifier(random_state=2017,activation='relu',solver='sgd',hidden_layer_sizes=(150,50))
...
print(grid_nn_estimator.best_score_)#80.00#83.05
print(grid_nn_estimator.best_params_)
print(grid_nn_estimator.score(X_train, y_train))#82.71#84.5
...
nn_scalar.fit(X_test)
X_test = nn_scalar.transform(X_test)

titanic_test['Survived'] = grid_nn_estimator.predict(X_test)
